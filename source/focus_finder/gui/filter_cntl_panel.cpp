...
void FilterCntlPanelT::updateProfile() {
    ...
    if (newFilterDevice) {
        mDeviceConnectedConnection = newFilterDevice->getParentDevice()->registerDeviceConnectedListener(
                [&]() { emit deviceConnectedSignal(); });
        mDeviceDisconnectedConnection = newFilterDevice->getParentDevice()->registerDeviceDisconnectedListener(
                [&]() { emit deviceDisconnectedSignal(); });
        mFilterPositionChangedConnection = newFilterDevice->registerFilterPositionChangedListener(
                [&](int currentPos) { emit filterPositionChangedSignal(currentPos); });
        mTargetPositionReachedConnection = newFilterDevice->registerTargetPositionReachedListener(
                [&](int targetPos) { emit targetPositionReachedSignal(targetPos); });
        mFilterMovementAbortedConnection = newFilterDevice->registerFilterMovementAbortedListener(
                [&](int currentPos) { emit filterMovementAbortedSignal(currentPos); });

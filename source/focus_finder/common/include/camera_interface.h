...
#define SOURCE_FOCUS_FINDER_COMMON_INCLUDE_CAMERA_INTERFACE_H_ SOURCE_FOCUS_FINDER_COMMON_INCLUDE_CAMERA_INTERFACE_H_
...
private:
    ...
    typedef signals2::signal<void(RectT<unsigned int>, std::shared_ptr<const ImageT>,
                                  bool)> ExposureCycleFinishedListenersT;
...
public:
    ...
    // Register on events
    signals2::connection
    registerExposureCycleFinishedListener(const ExposureCycleFinishedListenersT::slot_type &inCallBack) {
        return mExposureCycleFinishedListeners.connect(inCallBack);
    }

    template<class T>
    void unregisterExposureCycleFinishedListener(const T &inCallBack) {
    ...
    template<class T>
    void unregisterExposureCancelledListener(const T &inCallBack) {
        inCallBack.disconnect();
    }

    signals2::connection
    registerExposureDelayTimerUpdListener(const ExposureDelayTimerUpdListenersT::slot_type &inCallBack) {
        return mExposureDelayTimerUpdListeners.connect(inCallBack);
    }

    template<class T>
    void unregisterExposureDelayTimerUpdListener(const T &inCallBack) {
        inCallBack.disconnect();
    }
    ...
    template<class T>
    void unregisterExposureTimerUpdListener(const T &inCallBack) {
        inCallBack.disconnect();
    }
    ...
    template<class T>
    void unregisterFrameTransferUpdListener(const T &inCallBack) {
        inCallBack.disconnect();
    }

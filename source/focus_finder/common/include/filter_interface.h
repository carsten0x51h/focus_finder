...
public:
    ...
    */
    ...
    registerFilterPositionChangedListener(const FilterPositionChangedListenersT::slot_type &inCallBack) {
        return mFilterPositionChangedListeners.connect(inCallBack);
    }

    template<class T>
    void unregisterFilterPositionChangedListener(const T &inCallBack) {
        //mFilterPositionChangedListeners.disconnect(inCallBack);
        inCallBack.disconnect();
    }

    boost::signals2::connection
    registerTargetPositionReachedListener(const TargetPositionReachedListenersT::slot_type &inCallBack) {
        return mTargetPositionReachedListeners.connect(inCallBack);
    }

    template<class T>
    void unregisterTargetPositionReachedListener(const T &inCallBack) {
        //mTargetPositionReachedListeners.disconnect(inCallBack);
        inCallBack.disconnect();
    }

    boost::signals2::connection
    registerFilterMovementAbortedListener(const FilterMovementAbortedListenersT::slot_type &inCallBack) {
        return mFilterMovementAbortedListeners.connect(inCallBack);
    }

    template<class T>
    void unregisterFilterMovementAbortedListener(const T &inCallBack) {

...
void CentroidT::calcCentroid2ndMoment(const ImageT & inImg,
	...
	LOG(debug) << "CentroidT::calcCentroid2ndMoment - SIZE: " << SIZE << " --> subInImg - w=" << subInImg.width() << ", h=" << subInImg.height() << std::endl;
	...
	cimg_forXY(subInImg, x, y)
	{
		...
		dist[off] = sqrt(
				pow((double) distX[off], 2.0) + pow((double) distY[off], 2.0)); // distance of pixel from top-left, uses x and y to find hypotenuse
	}
...
std::optional<PointT<float> > CentroidT::calculate(const ImageT & inImg,
	...
	if (centroidFound) {
		LOG(info)
		<< "CentroidT::calculate() - centroid found - x: " << std::get<0>(relCenter) << ",y: " << std::get<1>(relCenter)
				<< std::endl;
	} else {
		LOG(info)

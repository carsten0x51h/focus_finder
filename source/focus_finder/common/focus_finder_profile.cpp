...
FocusFinderProfileT::load(const std::string &fullProfilePath, const fs::path &lightFramePath) {
    ...
    try {
        ...
        profile.setCameraDeviceName(
                pt.get(cameraSettingsPath + ".camera_device_name", defaults().getCameraDeviceName()));
         ...
         * 2. Focus
         */
        profile.setFocusDeviceName(
                pt.get("profile.settings.focus_settings.focus_device_name", defaults().getFocusDeviceName()));


        /**
         * 3. Filter
         */
        profile.setFilterDeviceName(
                pt.get("profile.settings.filter_settings.filter_device_name", defaults().getFocusDeviceName()));
        ...
        profile.setEnableStarAutoTracking(
                pt.get(imagingPath + ".enable_star_auto_tracking", defaults().getEnableStarAutoTracking()));
        profile.setNumFramesPerFocusPos(
                pt.get(imagingPath + ".num_frames_per_focus_pos", defaults().getNumFramesPerFocusPos()));
        profile.setStarDetectionSnrBoundary(
                pt.get(imagingPath + ".star_detection_snr_boundary", defaults().getStarDetectionSnrBoundary()));
        profile.setFocusingFilterPos(pt.get(imagingPath + ".focusing_filter_pos",
                                            defaults().getFocusingFilterPos()));  // NOTE: -1 = do not change / set
        profile.setEnableCameraCooler(
                pt.get(imagingPath + ".enable_camera_cooler", defaults().getEnableCameraCooler()));
        profile.setCameraCoolerTargetTemperature(pt.get(imagingPath + ".camera_cooler_target_temperature",
                                                        defaults().getCameraCoolerTargetTemperature()));
        ...
        profile.setLimitFocusMeasureType(
                pt.get(focusCurveRecordingPath + ".limit_focus_measure_type", defaults().getLimitFocusMeasureType()));
        profile.setCurveFocusMeasureType(
                pt.get(focusCurveRecordingPath + ".curve_focus_measure_type", defaults().getCurveFocusMeasureType()));
        profile.setStepSize(pt.get<int>(focusCurveRecordingPath + ".step_size")); // Explicitely no default
        profile.setNumberCurvesToRecord(
                pt.get(focusCurveRecordingPath + ".num_curves_to_record", defaults().getNumberCurvesToRecord()));
        profile.setFocusMeasureLimit(
                pt.get(focusCurveRecordingPath + ".focus_measure_limit", defaults().getFocusMeasureLimit()));
        ...
        profile.setFocusFinderCalibration(
                FocusFinderCalibrationT::load(pt.get_child("profile.calibration"), curveFitParms, lightFramePath));
...
void FocusFinderProfileT::save(const std::string &fullProfilePath, const fs::path &lightFramePath,
    ...
    try {
        ...
        pt.put<int>(imagingPath + ".focusing_filter_pos",
                    profile.getFocusingFilterPos()); // NOTE: -1 = do not change / set
        ...
        pt.put<typename FocusMeasureTypeT::TypeE>(focusCurveRecordingPath + ".limit_focus_measure_type",
                                                  profile.getLimitFocusMeasureType());
        pt.put<typename FocusMeasureTypeT::TypeE>(focusCurveRecordingPath + ".curve_focus_measure_type",
                                                  profile.getCurveFocusMeasureType());

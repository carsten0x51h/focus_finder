...
std::shared_ptr<FocusCurveRecordT> FocusControllerT::measureFocus() {
    ...
    while (!success && retry < retries) {
        ...
        try {
            ...
            LOG(debug) << "FocusControllerT::measureFocus... expTime: " << expTime.count() << " -> inMs: "
                       << inMs.count() << std::endl;
            ...
            while (sampleNo <= numSamples) {
                ...
                if (starTrackingEnabled) {
                    LOG(debug)
                        << "FocusControllerT::measureFocus... Star tracking enabled... setting focus star position to new centroid position "
                        << newCentroidAbsRoiCoords << std::endl;
            ...
            LOG(warning) << "Unable to calculate FWHM... re-taking image... (retrying " << retry << "/" << retries
                         << ")..." << std::endl;
            success = false;
        }
    } // end while
...
void FocusControllerT::boundaryScanLinear(const SelfOrientationResultT &selfOrientationResult, float stepSize,
                                          float focusMeasureLimit) {
    using namespace std::chrono_literals;
    ...
    while (!limitCrossed) {
        ...
        LOG(debug) << "FocusControllerT::linearBoundaryScan... pos: " << getFocus()->getCurrentPos()
                   ...
                   << ", progressPerc: " << progressPerc << std::endl;
...
int FocusControllerT::boundaryScanWithFocusCurveSupport(const std::shared_ptr<CurveFunctionT>& focusCurveFunction,
    ...
    LOG(debug) << "FocusControllerT::boundaryScanWithFocusCurveSupport..."
               ...
               << " -> limit focus measure range=[" << lowerFocusMeasure << ", " << upperFocusMeasure << "]"
               << std::endl
               << " -> estimated rel position range=[" << lowerRelLimitPos << ", " << upperRelLimitPos << "]"
               << std::endl
               << " -> current focus measure=" << focusMeasure << " -> estimated rel. pos=" << estimatedRelPos
               << std::endl
    ...
    if (boundaryLoc != BoundaryLocationT::WITHIN_BOUNDARY_RANGE) {
        ...
        BoundaryLocationT::TypeE boundaryLoc2 = determineBoundaryLoc(lowerFocusMeasure, upperFocusMeasure,
                                                                     focusMeasure2);
        ...
        if (goalOvershot) {
            LOG(debug)
                << "FocusControllerT::boundaryScanWithFocusCurveSupport...goal overshot... inverting direction..."
                << std::endl;
        ...
        while (boundaryLoc2 != BoundaryLocationT::WITHIN_BOUNDARY_RANGE) {
            ...
            if (goalOvershot) {
                ...
                LOG(debug)
                    << "FocusControllerT::boundaryScanWithFocusCurveSupport...goal overshot... inverting direction to "
                    << FocusDirectionT::asStr(dir)
...
SelfOrientationResultT FocusControllerT::performSelfOrientation(float focusMeasureLimit) {
    ...
    if (focusMeasure2 > focusMeasure1) {
        ...
        LOG(error)
            << "FocusControllerT::determineTargetFocusDirection... Unable to determine target focus direction. No difference in focus measure detected."
            << std::endl;

        // TODO: reporting?
        throw FocusControllerCancelledExceptionT(
                "Unable to determine target focus direction. No difference in focus measure detected.");
    }

    LOG(info) << "Successfully performed self-orientation using parameters: " << std::endl
              << " > stepFactor: " << STEP_FACTOR << " -> self orientation step size: " << selfOrientationStepSize
              << std::endl
